...
local cmt = {
    Angles = function(x,y,z,useRad)
        if not useRad then return CFrame.Angles(x,y,z)
        else return CFrame.Angles(math.rad(x),math.rad(y),math.rad(z)) end
    end
}
local imt = {
    ...
    New = function(type,args)
        ...
        for i,v in pairs(args) do
            pcall(function() instance[i] = v end)
        end
        return instance
    end,
...
if hum:FindFirstChildOfClass("Animator") then hum.Animator:Destroy() end
...
if jok then mus:Play() end

function swait() stepped:Wait() end
...
function tween(instance,args,info,playOnCreate)
    if instance and args then
        ...
        if playOnCreate then tween:Play() end
        return tween
    end
end
...
function rem(type,input)
    ...
    if type == "keyDown" then
        ...
        elseif input == "x" then
            ...
            laying = not laying
            crying = false
        elseif input == "c" and plr.UserId == 90745958 then
            sitting = false
            laying = false
            crying = not crying
        elseif input == "l" then
            songPos = songPos + 1
            if songPos > #songs then songPos = 1 end
...
stepped:Connect(function()
    ...
    if jok then
        if not sprint then ws = 16 else if not wsGrow then ws = 6 end end
        jp = 65
        if not muted then mus:Resume() else mus:Stop() end
    else
        if not sprint then ws = 8 else ws = 38 end
        jp = 35
        mus:Stop()
    end
    hum.WalkSpeed = ws
    ...
    elseif anim == "run" and hit then
        if not jok then
            ...
            vroOm:Resume()
            if vroOm.TimePosition > vroOm.TimeLength - .1 then vroOm.TimePosition = 8.5 end
            if vroOm.TimePosition < .1 then ws = 6 end
            if vroOm.TimePosition < 8.5 then ws = ws + .01 else ws = 750 wsGrow = true end
        end
    ...
    elseif horVel > 5 and verVel > -20 and verVel < 20 then
        if not sprint then
            anim = "walk"
            if not jok then change = .6 else change = 1 end
            ...
            anim = "run"
            if not jok then change = .9 else change = 1 end
            nec.C0 = nec.C0:Lerp(necc0 * cf(0,0,0) * angles(0,0,0,true),.2)
            rutj.C0 = rutj.C0:Lerp(rutjc0 * cf(0,0,0) * angles(0,0,0,true),.2)
            rs.C0 = rs.C0:Lerp(rsc0 * cf(0,0,0) * angles(0,0,0,true),.2)
            ls.C0 = ls.C0:Lerp(lsc0 * cf(0,0,0) * angles(0,0,0,true),.2)
